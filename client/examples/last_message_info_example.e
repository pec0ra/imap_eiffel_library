...
feature -- Basic operation
	...
	run_example
		...
		do
			io.put_new_line
			print("** Login, select INBOX and print the second message **")
			io.put_new_line
			io.put_new_line
		...
		get_last_message
			...
			do
				...
				messages := imap.fetch (create {IL_SEQUENCE_SET}.make_from_natural_range (2, 2), data_items)
				...
				loop
					...
					field := message.item.data.at (From_field)
					if attached {STRING} field then
					...
					field := message.item.data.at (Date_field)
					if attached {STRING} field then
						create parser.make_from_text (field)
						print ("Date : ")
					...
					field := message.item.data.at (Subject_field)
					if attached {STRING} field then
						create parser.make_from_text (field)
						print ("Subject : ")
					...
					print ("Body text :")
					io.put_new_line
					print (message.item.data.at (Body_text_field))
					io.put_new_line
